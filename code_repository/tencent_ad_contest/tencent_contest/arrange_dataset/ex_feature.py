...
def load_hashtable(hashtable_path, assemble):
    ...
    label_ary = assemble.loc[:, 'label'].values
...
def extract_feature(assemble_now, hashtable, index):
    ...
    print('[{}]train dataset shape='.format(index + 1), train_fea.shape)
    print('[{}]pred dataset shape='.format(index + 1), pred_fea.shape)

    scipy.sparse.save_npz(root_path + 'train_{}.npz'.format(index), train_fea)
    scipy.sparse.save_npz(root_path + 'pred_{}.npz'.format(index), pred_fea)
    label_ary.to_csv(root_path + 'label_{}.csv'.format(index), index=False)
...
def main():
    ...
    cnt = 20
    train_df = assemble.loc[assemble['label'] != -1, :].reset_index(drop=True)
    pred_df = assemble.loc[assemble['label'] == -1, :].reset_index(drop=True)
    step_len = int(train_df.shape[0] / cnt)

    hashtable = load_hashtable(root_path + 'hashtable.txt', assemble)
    for i in range(7, cnt):
        print('{}/{}'.format(i + 1, cnt))

        start = step_len * i
        end = min((i + 1) * step_len, train_df.shape[0])
        assemble_now = pd.concat([train_df[start:end], pred_df]).reset_index(drop=True)
